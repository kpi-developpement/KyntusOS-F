...
export default function TaskDispatchPage() {
    ...
    const init = async () => {
      try {
        const [p, t] = await Promise.all([
          fetch("http://localhost:3000/api/users/pilots").then(r => r.json()),
          fetch("http://localhost:3000/api/templates").then(r => r.json())
        ]);
  ...
  useEffect(() => {
    if (mode === 'MANUAL' && manualTemplateId) {
        setLoading(true);
        fetch(`http://localhost:3000/api/tasks/unassigned/${manualTemplateId}`)
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/api/tasks/columns/${smartTemplateId}`)
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/api/tasks/values/${smartTemplateId}/${selectedColumnKey}`)
  ...
  const executeDispatch = async () => {
    ...
    try {
        const res = await fetch("http://localhost:3000/api/dispatch/execute", {
        ...
        if(res.ok) {
            ...
            if(mode === 'MANUAL') {
                 const ref = await fetch(`http://localhost:3000/api/tasks/unassigned/${manualTemplateId}`).then(r=>r.json());
                 setTasks(ref);
            }
