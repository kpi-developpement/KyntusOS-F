...
export default function AuditPage() {
  ...
  const loadStats = () => {
    fetch("http://localhost:3000/api/admin/pilots-stats")
      .then(res => res.json())
      .then(data => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
        const res = await fetch(`http://localhost:3000/api/admin/search?eps=${epsSearch}`);
        if (res.ok) {
  ...
  const handleFlagError = async () => {
    ...
    try {
        const res = await fetch(`http://localhost:3000/api/admin/flag-error/${foundTask.id}`, {
            method: "POST"
        });
