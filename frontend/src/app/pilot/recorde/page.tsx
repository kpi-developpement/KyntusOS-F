...
export default function PilotRecordsPage() {
  ...
  const changeBgTheme = useCallback((color: string) => {
    if (bgOverlayRef.current) bgOverlayRef.current.style.backgroundColor = color;
  }, []);
  ...
  const fetchAlertsData = useCallback(async (cat: string, yr: number, mo: number) => {
    ...
    setValidationAlerts([]); setCommentAlerts([]);
    setShowValidationDetails(false); setShowCommentDetails(false);
    try {
  ...
  const handleUpload = async (filesToUpload: File[], uploadYear: number, uploadMonth: number, uploadCategory: string) => {
    ...
    try {
      const sortedFiles = [...filesToUpload].sort((a, b) => getFileRank(a.name) - getFileRank(b.name));
      for (let i = 0; i < sortedFiles.length; i++) {
        const file = sortedFiles[i];
        setUploadProgressText(`INJECTING PAYLOAD ${i + 1}/${sortedFiles.length} : ${file.name}`);
        const formData = new FormData(); 
        formData.append("file", file); 
        await fetch(`${API_BASE}/api/pilot-records/import/1?year=${uploadYear}&month=${uploadMonth}&category=${uploadCategory}`, { method: "POST", body: formData });
      }
    ...
    } catch (error) {} finally { setIsUploading(false); setUploadProgressText(""); }
  };
  ...
  return (
    ...
    >
      ...
      <div className="tilt-panel" style={{ maxWidth: "1400px", margin: "0 auto" }}>
          ...
          <>
            {!isScanningAlerts && validationAlerts.length > 0 && (
              <div 
                ...
                style={{ position: "relative", zIndex: 10, marginBottom: "1.5rem", cursor: "pointer", border: "1px solid #ff003c", borderLeft: "6px solid #ff003c", padding: "1.2rem", color: "#ffb3b3", fontFamily: "monospace", animation: "blink-alert-red 1.5s infinite", backdropFilter: "blur(15px)", borderRadius: "6px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                    ...
                    <div>
                      <h3 style={{ margin: 0, fontSize: "1.1rem", fontWeight: "900", letterSpacing: "2px", color: "#fff", animation: "glitch-text 3s infinite", textShadow: "0 0 10px #ff003c" }}>CRITICAL STATUS BREACH</h3>
            ...
            {!isScanningAlerts && commentAlerts.length > 0 && (
              <div 
                onClick={() => setShowCommentDetails(!showCommentDetails)}
                className="hardware-accelerated target-bracket"
                style={{ position: "relative", zIndex: 10, marginBottom: "1.5rem", cursor: "pointer", border: "1px solid #f59e0b", borderLeft: "6px solid #f59e0b", padding: "1.2rem", color: "#fef3c7", fontFamily: "monospace", animation: "blink-alert-orange 2.5s infinite", backdropFilter: "blur(15px)", borderRadius: "6px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
        ...
        {/* 🚀 TABLEAU TOUJOURS VISIBLE, MÊME S'IL EST VIDE 🚀 */}
      ...
      <FileUploadModal 
        ...
        onFileDeleted={() => {
            setSuccessMsg("🗑️ Target payload eliminated.");
            setTimeout(() => setSuccessMsg(""), 5000);
            setRefreshTrigger(prev => prev + 1); 
        }}
      />
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(15, 23, 42, 0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999999, backdropFilter: "blur(8px)" }}>
          <div className="target-bracket" style={{ backgroundColor: "rgba(6, 11, 25, 0.95)", border: "1px solid #ff00ff", borderRadius: "8px", width: "700px", maxWidth: "95%", maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 50px rgba(0,0,0,0.8)" }}>
            ...
            <div style={{ padding: "1.5rem", display: "flex", gap: "15px", backgroundColor: "rgba(10, 15, 30, 0.8)", borderBottom: "1px solid rgba(148,163,184,0.2)" }}>
              {/* 🔥 FIX: onChange kay-beddel historyEpsInput, machi epsInputValue 🔥 */}
              <input 
